...
if [ -z "$KSS_PUBLIC_DNS_NAME" ]
then
  if [ "$SERVER_FACTORY" == "https" ]
  then
    KSS_PUBLIC_DNS_NAME="$(cat /etc/host_fqdn)"
  else
    KSS_PUBLIC_DNS_NAME="localhost"
  fi
fi
...
LOCAL_BASE_URL="${HTTP_PROTO_PREFIX}://${KSS_PUBLIC_DNS_NAME}:3000/"
GLOBAL_BASE_URL="${HTTP_PROTO_PREFIX}://${KSS_PUBLIC_DNS_NAME}${USED_CSS_PORT_SUFFIX}/"
...
#       sleep 0.1
  ...
  then
    ...
    for wait in $(seq 1 120)  # wait max 2 minutes, then just give up
    do
      if echo -n | openssl s_client -connect "${KSS_PUBLIC_DNS_NAME}:443" -verify_return_error > /dev/null 2>&1;
      then
        echo "      OK: Got a valid certificate from ${KSS_PUBLIC_DNS_NAME}:443"
        _CSS_CERT_READY=true
        sleep 0.2
        break
      else
        echo "      Not (yet) OK: Failed to get valid cert on ${KSS_PUBLIC_DNS_NAME}:443"
      fi
  ...
  echo
  echo -n "   Test CSS at ${HTTP_PROTO_PREFIX}://${KSS_PUBLIC_DNS_NAME}:${_USED_CSS_PORT}/ ..."
#  echo -e "GET ${KSS_PUBLIC_DNS_NAME}/ HTTP/1.1\nHost: selftest\nConnection: close\n\n" | tee /dev/stdout | openssl s_client -connect "${KSS_PUBLIC_DNS_NAME}:443" -quiet
  _CSS_TEST_OUTPUT="$(curl -s -I "${HTTP_PROTO_PREFIX}://${KSS_PUBLIC_DNS_NAME}:${_USED_CSS_PORT}/" || true)"

  if ! echo "${_CSS_TEST_OUTPUT}" | grep -i -q 'x-powered-by: Community Solid Server'
  then
    echo " FAILED"
    echo 'ERROR: CSS Test failed.'
    echo "       Ran command: curl -s -I ${HTTP_PROTO_PREFIX}://${KSS_PUBLIC_DNS_NAME}:${_USED_CSS_PORT}/"
...
#  cp -v "/etc/systemd/system/css.service.template" /etc/systemd/system/
  sed -e "s/<<CSS_DNS_NAME>>/${KSS_PUBLIC_DNS_NAME}/g" \
...
#  AUTHORIZATION_ARG='--add-acl-files'
  ...
  set -x
  css-populate --url "${HTTP_PROTO_PREFIX}://${KSS_PUBLIC_DNS_NAME}${USED_CSS_PORT_SUFFIX}" \
      --generate-users \
...
function collect_access_tokens() {
  ...
  css-flood --url "${HTTP_PROTO_PREFIX}://${KSS_PUBLIC_DNS_NAME}${USED_CSS_PORT_SUFFIX}" --duration 1 --userCount "${CONTENT_USER_COUNT}" --parallel 1 \
...
then
  ...
  then
    ...
    certbot run --nginx --domain "${KSS_PUBLIC_DNS_NAME}" --agree-tos --register-unsafely-without-email
...
server {
  ...
  server_name ${KSS_PUBLIC_DNS_NAME};
